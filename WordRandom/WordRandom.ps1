...
$wordList = Get-Content .\WordRandom.txt
...
while ( $true ) {
    ...
    if (( $mode -notmatch '[12345]' ) -or ( $mode.Length -ne 1 )) {
...
while ( $true ) {
    ...
    if ( $notMin ) {
        Write-Warning "You have to use at least $( $minLetters ) of the randomly chosen letters.`n"
        $tries++
